pipeline {
  ...
  environment {
    ...
    awsecrRegistry = '412144898273.dkr.ecr.ap-northeast-2.amazonaws.com/eks-demo-repo'
  ...
  stages {
    ...
    stage('Checkout Application Git Branch') {
      steps {
        checkout([$class: 'GitSCM', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: githubCredential, url: 'https://github.com/boulde/cicd.git']]])
      }
      // steps 가 끝날 경우 실행한다.
    ...
    stage('Deploy') { 
      steps {
        ...
        sh "git push https://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@github.com/boulde/cicd.git"
      }
      post {
        failure {
